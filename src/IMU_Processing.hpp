    ...
    : b_first_frame_(true), imu_need_init_(true), start_timestamp_(-1)
{
    ...
    cov_bias_gyr = Eigen::Vector3d(0.0001, 0.0001, 0.0001);
    cov_bias_acc = Eigen::Vector3d(0.0001, 0.0001, 0.0001);
...
void ImuProcess::set_gyr_cov(const Eigen::Vector3d &scaler)
{
    cov_gyr_scale = scaler;
}

void ImuProcess::set_acc_cov(const Eigen::Vector3d &scaler)
{
    cov_acc_scale = scaler;
}

void ImuProcess::set_gyr_bias_cov(const Eigen::Vector3d &b_g)
{
    cov_bias_gyr = b_g;
}

void ImuProcess::set_acc_bias_cov(const Eigen::Vector3d &b_a)
{
    cov_bias_acc = b_a;
}
...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
    ...
    // state_inout.rot = Eye3d; // Exp(mean_acc.cross(Eigen::Vector3d(0, 0, -1 / scale_gravity)));
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI &pcl_out)
{
    ...
    for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--)
    {
        ...
        for (; it_pcl->curvature / double(1000) > head->offset_time; it_pcl--)
        {
            ...
            Eigen::Vector3d P_compensate = imu_state.offset_R_L_I.conjugate() * (imu_state.rot.conjugate() * (R_i * (imu_state.offset_R_L_I * P_i + imu_state.offset_T_L_I) + T_ei) - imu_state.offset_T_L_I); // not accurate!
